...
\textbf{Linear Network: }\\
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/linear_cartpole.png}
\end{center}
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/linear_mountaincar.png}
\end{center}

I'm able to solve MountainCar-v0 with linear network. Because there is a linear policy to reach the goal.(Go left when at left cliff, go right when at right cliff.)
For CartPole-v0, the network has the trend to converge, but it takes too many iterations. I think linear network can also solve this envirment, because there is a linear policy that cart goes left when it leans left, goes right when it leans right to keep balance.\\

\textbf{Experience Replay: }\\
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/experience_cartpole.png}
\end{center}
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/experience_mountaincar.png}
\end{center}

Here I set batch size as 32, Memory size as 50000, burn in size as 10000.
After adding experience replay, I can solve both evironments in a reasonable time.\\
But the training is not stable, the reward will diverge after it reach the goal.\\

\textbf{Deep Q Network (MLP): }\\
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/MLP_cartpole.png}
\end{center}
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/MLP_mountaincar.png}
\end{center}

For the network architecture, I used one input layer which takes state, one hidden layer with 20 units and one output layer which corresbonds to the Q values to each state. 
I used Adam as the optimizer to minimize the loss.
Also I included experience replay in this model.\\
\\
For both evironments, the model increased the convergence rate and stability. The reward could be converged within less iterations. But for MountainCar-v0, the average rewards get decreased a little bit.\\

\textbf{Dueling Network: }\\
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/duel_cartpole.png}
\end{center}
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/duel_mountaincar.png}
\end{center}

After adding the dueling layer, both envirments get converged with less iterations. The training becomes more stable and is less likely to diverge.\\
...
\textbf{SpaceInvader: }\\
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/spaceinvader.png}
\end{center}

For the network architecture, I combined convolutional neuro network with dueling layer.\\
The raw input is $210 \times 160$ pixels with 128 possible colors.\\
I convert the images to grayscale and then resize the images to 84 x 84 pixels.\\
...
Between input and output, there are 16 $8 \times 8$ con2d, ReLU $\to$ $2 \times 2$ maxpool $\to$ 32 $4 \times 4$ con2d, ReLU $\to$ $2 \times 2$ maxpool $\to$ 256 full connection, ReLU $\to$ Dropout $\to$ Dueling layer $\to$ Q values.\\

\textbf{Reward Table: }\\
$R_{avg}$ is average total reward per episode in 100 episodes by fully trained model.
\begin{center}
  \begin{tabular}{|c|c|c|c|}\hline
    {\bf Model} & {\bf $R_{avg}$ in CartPole-v0 } & {\bf $R_{avg}$ in  MountainCar-v0} \\ \hline
    linear Q-network & $34.48 \pm 13.35$ & $-121.49 \pm 9.87$ \\ \hline
    {\shortstack{linear Q-network with  \\ experience replay }} & $200 \pm 0$ & $-130.97 \pm 19.84$ \\ \hline
    MLP DQN & $200 \pm 0$ & $-134 \pm 17.45$ \\ \hline
    Dueling MLP DQN & $200 \pm 0$ & $-150.27 \pm 35$ \\ \hline
  \end{tabular}
\end{center}
\begin{center}
  \begin{tabular}{|c|c|c|c|}\hline
    {\bf Model} & {\bf $R_{avg}$ in SpaceInvader-v0 } \\ \hline
    CNN DQN & $273.3 \pm 125.32$\\ \hline
  \end{tabular}
\end{center}

\textbf{Quadratic Loss Function: }\\
For all the network, I use mean squared error as loss function. This is equivalent to vanilla Q-learning update.\\
...
The loss function use mean squared error. To minimize the loss function, we need to adjust the weight vector by a small amount at each step in the direction that would most reduce the error. To do this, we need to get the gradiant by calculating the derivative with respect to $w$.\\
\\
So we have:\\
	$\\ \nabla \ell (w) = \nabla _w(r + \gamma \max _{a' \in A} Q _w(s', a') - Q _w(s, a)) ^2\\
	= 2(r + \gamma \max _{a' \in A} Q _w(s', a') - Q _w(s, a))\nabla _w (r + \gamma \max _{a' \in A} Q _w(s', a') - Q _w(s, a))\\
	= - 2(r + \gamma \max _{a' \in A} Q _w(s', a') - Q _w(s, a))\nabla _w Q _w(s, a)$\\


Where $\alpha \in \mathbb R$ is the learning rate. Thus we can update $w$ as:\\

$w := w - \frac{1}{2}\alpha \nabla \ell (w) = w + \alpha (r + \gamma \max _{a' \in A} Q _w(s', a') - Q _w(s, a))\nabla _w Q _w(s, a)$\\
